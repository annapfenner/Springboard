...
SELECT COUNT(name) AS count_facilities_no_cost_members FROM `Facilities` WHERE membercost = 0.0;
...
SELECT *
FROM 
	(SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname) AS first_last_name, f.name, b.starttime AS startdate, CASE WHEN b.memid =0 THEN f.guestcost * b.slots
	ELSE f.membercost * b.slots 
	END AS member_or_guest_cost
	FROM Bookings AS b, Facilities AS f, Members AS m 
    WHERE b.facid = f.facid
	AND b.memid =m.memid) AS sub
